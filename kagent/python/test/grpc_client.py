...
def predict(host, port, hostname, model, signature_name, input_path):
    ...
    for k,v in md.items():
        if k == "Active_Power":
            continue
        if k in ["Month", "Day", "Day_Of_Week", "Hour"]:
            request.inputs[k].CopyFrom(make_tensor_proto([v], dtype=tf.int64, shape=[1, 1]))
        else:
            request.inputs[k].CopyFrom(make_tensor_proto([v], dtype=tf.float64,shape=[1, 1]))
