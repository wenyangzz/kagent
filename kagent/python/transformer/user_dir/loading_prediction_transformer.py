...
RAW_COLUMN_NAME = ['wind_speed','weather_temperature_celsius','weather_relative_humidity','global_horizontal_radiation',\
...
class MyTransformer(object):
    ...
    def feature_work(self, raw_data: List, cols_name: List, type_info: List, value_info: Dict):
        ...
        # 根据模型定义，进行维度变换: (n,) -> (1,n,)
        md = {}
        for n,v in zip(range(0, 96), raw_data):
            md['fea_' + str(n)] = v
        # 特征变换
        for f,t in type_info:
            ...
            if t == 'category':
                ...
                if f in md and md[f] is not None: # 最大最小标准化
                    res.append((float(md[f])-minv)/(maxv-minv))
                else: # 缺失值使用最小值填充
                    res.append(minv)
        # 根据模型定义，这里需要进行维度变换: (n,) -> (1,n,)
        res = [res]
        return res
    ...
    def load_feature_value(self, feature_value_file: str):
        ...
        """
        fea_info = {}
        if not os.path.isfile(feature_value_file):
            return fea_info
    ...
    def load_feature_type(self, feature_type_file: str):
        ...
        """
        fea_type = []
        if not os.path.isfile(feature_type_file):
            return fea_type
    ...
    def postprocess(self, inputs: Dict) -> Dict:
        ...
        if 'target' in self.fea_value and 'predictions' in inputs and inputs['predictions']:
            result = {'predictions': []}
            minv = self.fea_value['target']['min']
            maxv = self.fea_value['target']['max']
            data = inputs['predictions']
            for md in data:
                res = map(lambda x: x*(maxv-minv)+minv, md)
                result['predictions'].append(list(res))
            return result
        '''
